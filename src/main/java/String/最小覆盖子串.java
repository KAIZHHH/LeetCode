...
public class 最小覆盖子串 {
    ...
    public static String minWindow(String s, String t) {
        ...
        int count = t.length(); //所需要的总个数 最大是t.length 一个还没有的时候
        int start = 0;//size记录窗口大小，start是最小覆盖串开始的index
        //遍历所有字符
        while (r < s.length()) {
            //表示t中包含当前遍历到的这个c字符，更新目前所需要的count数大小，应该减少一个
            if (need[s.charAt(r)] > 0) {//遍历到的 s中的c字符 是所需要的！
                count--;      //所需要的个数-1
            }
            need[s.charAt(r)]--;//所需要c字符的次数 -1


            if (count == 0) {//窗口中已经包含所有字符 开始过滤掉多余元素
                //左指针遍历的字符在need数组中次数如果是负数表明当前字符在窗口中是多余的，需要过滤掉
